...
class ChordDisplay {
    ...
    formatChordSymbol(symbol) {
        ...
        formatted = formatted.replace(/♭/g, '<span class="accidental">♭</span>');
        formatted = formatted.replace(/ø/g, '<span class="quality-symbol">ø</span>');
        formatted = formatted.replace(/dim/g, '<span class="quality-symbol">dim</span>');
        return formatted;
    }
    ...
    formatNotesList(notes) {
        ...
        const formattedOthers = otherNotes.map((note) => this.formatNote(note)).join(", ");
    ...
    clearDisplay() {
        this.container
            .querySelectorAll(".chord-symbol, .chord-quality, .chord-extensions, .chord-notes")
